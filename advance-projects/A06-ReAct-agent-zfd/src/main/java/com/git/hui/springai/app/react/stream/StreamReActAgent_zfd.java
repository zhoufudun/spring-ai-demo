 ...
 * @date 2026/3/4
 */
public class StreamReActAgent_zfd {
    ...
    public StreamReActAgent_zfd(ChatClient chatClient, List<ToolCallback> tools) {
...
//        ToolCallingChatOptions.builder().internalToolExecutionEnabled(Boolean.FALSE).build();
        ...
        responseFlux.doOnNext(chatResponse -> {
            if (chatResponse.getResult() != null && chatResponse.getResult().getOutput() != null) {
                ...
                if (assistantMessage.getToolCalls() != null && !assistantMessage.getToolCalls().isEmpty()) {
                    log.info("【检测到工具调用】:" + assistantMessage.getToolCalls().stream().map(tc -> tc.name()).toList());
                }

                // 保存最后一个消息
                lastMessage.set(assistantMessage);
            }
        }).blockLast(); // 这个阻塞当前线程，流获取全部结果
