...
/**
 * 原文地址：
 * https://ppai.top/ai-guides/ai-dev/%E8%BF%9B%E9%98%B6%E7%AF%87/A05.%E4%BD%BF%E7%94%A8LangGraph4J%E5%AE%9E%E7%8E%B0Agent%E8%B7%AF%E7%94%B1%E9%80%89%E6%8B%A9.html#_1-%E5%B7%A5%E4%BD%9C%E6%B5%81%E7%A8%8B
 */
// 内部类：构建并执行 StateGraph 的服务
public class WeatherRecommendAgent_zfd {

    private final ChatClient chatClient;
    private final CompiledGraph<AgentState> graph;

    public WeatherRecommendAgent_zfd(ChatClient chatClient) throws GraphStateException {
    ...
    private CompiledGraph<AgentState> initGraph(String location) throws GraphStateException {
        ...
        NodeAction weatherNode = new NodeAction() {
            @Override
            public Map<String, Object> apply(AgentState agentState) throws Exception {
                // 取入参 location（状态里可能已有）
                String loc = (String) agentState.value("location").orElseGet(() -> location);
                ...
                return Map.of("location", loc, "weather", weather);
            }
        };

        // Node2: router - 只是做路由，本节点不做 State 的任何更新
        NodeAction<AgentState> routerNode = new NodeAction<>() {
            @Override
            public Map<String, Object> apply(AgentState agentState) throws Exception {
                // 这个节点，用于模拟啥也不干的场景
                String w = (String) agentState.value("weather").get();
        ...
        NodeAction<AgentState> outdoorNode = new NodeAction<>() {
            @Override
            public Map<String, Object> apply(AgentState agentState) throws Exception {
                String loc = (String) agentState.value("location").orElseGet(() -> location);
                String weather = (String) agentState.value("weather").orElseGet(() -> "晴天");

                String prompt = String.format("你是一个资深旅行推荐师：用户在地点“%s”，当前天气“%s”。请用中文给出 3 个适合外出（户外）游玩的项目，每个项目写一行：项目名称 - 30 字以内简短描述 - 预计耗时。不要写多余开头语，返回纯文本列表。",
                        loc, weather);
                String result = chatClient.prompt().user(prompt).call().content();
                System.out.println("[outdoorNode] model result:\n" + result);

                return Map.of("outdoor_recommendations", result);
            }
        };
        ...
        // Build StateGraph
        CompiledGraph compile = new StateGraph<>(stringObjectMap -> new AgentState(stringObjectMap))
                ...
                .addConditionalEdges("router",
                        new RouteEvaluationResult(), EdgeMappings.builder()
                                .to("outdoor", "go_outdoor")
                                .to("indoor", "go_indoor")
                                .toEND()
                                .build())
