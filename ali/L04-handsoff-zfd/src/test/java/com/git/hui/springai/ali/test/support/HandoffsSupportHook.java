...
public class HandoffsSupportHook extends ModelHook {
    ...
    private static final String WARRANTY_COLLECTOR_PROMPT = """
            ...
            1. 热情地迎接客户
            2. 询问他们的设备是否在保修期内
            3. 使用record_warranty_status来记录他们的回复，并进入下一步
            保持对话式且友好。不要一次问太多问题。
            """;
    ...
    private static final String ISSUE_CLASSIFIER_PROMPT = """
            ...
            在这一步，你需要：
            1. 请客户描述他们的问题
            2. 判断是硬件问题（物理损坏、部件断裂）还是软件问题（应用程序崩溃、性能问题）
            3. 使用record_issue_type来记录分类，并进入下一步
    ...
    private static final String RESOLUTION_SPECIALIST_PROMPT = """
            ...
            在这一步，你需要：
            1. 对于软件问题：使用provide_solution提供故障排除步骤
            2. 对于硬件问题：
               - 如果在保修期内：使用provide_solution解释保修流程
               - 如果超出保修期：上报给人工客服，以获取付费维修选项
    ...
    public HandoffsSupportHook() {
        ...
        this.stepConfigInterceptor = new StepConfigInterceptor(Map.of(
                SupportStateConstants.STEP_WARRANTY_COLLECTOR, new StepConfigInterceptor.StepConfig(WARRANTY_COLLECTOR_PROMPT, List.of(recordWarranty), List.of()),
                SupportStateConstants.STEP_ISSUE_CLASSIFIER, new StepConfigInterceptor.StepConfig(ISSUE_CLASSIFIER_PROMPT, List.of(recordIssueType), List.of(SupportStateConstants.WARRANTY_STATUS)),
                SupportStateConstants.STEP_RESOLUTION_SPECIALIST, new StepConfigInterceptor.StepConfig(RESOLUTION_SPECIALIST_PROMPT, List.of(provideSolution, escalateToHuman), List.of(SupportStateConstants.WARRANTY_STATUS, SupportStateConstants.ISSUE_TYPE))));
    }
