...
import static com.git.hui.springai.ali.express.ExpressOrderStateConstants.*;
 ...
 */
@Slf4j
public class ExpressOrderTools_zfd {
    private static final ExpressOrderTools_zfd INSTANCE = new ExpressOrderTools_zfd();

//    private static final List<ToolCallback> TOOLS = Arrays.asList(ToolCallbacks.from(INSTANCE));

    private static final List<ToolCallback> TOOL_CALLBACK_LIST = Arrays.asList(MethodToolCallbackProvider.builder().toolObjects(INSTANCE).build().getToolCallbacks());

    /**
     * 获取所有工具回调
     */
    public static List<ToolCallback> getTOOLS() {
        return TOOL_CALLBACK_LIST;
    }
...
//    }
    public static ToolCallback findByName(String name) {
        return TOOL_CALLBACK_LIST.stream().filter(tc -> name.equals(tc.getToolDefinition().name())).findFirst().orElseThrow(() -> new IllegalArgumentException("Unknown tool: " + name));
    }
...
//    }
    ...
    public String receiveAddress(@ToolParam(description = "收件人详细信息，包括姓名、电话、详细地址信息等") String receiveInfo
        ...
        // 更新状态到2步（STEP_SEND_INFO_COLLECTOR）
        updateStep(toolContext, 2);
     ...
     * 第二步：接收并保存发件人信息
     */
    @Tool(description = "保存快递的发货人信息，用于快递下单场景")
    public String sendAddress(
        ...
        // 保存到上下文中
        saveInfoToContext(toolContext, KEY_SEND_INFO, sendInfo);
        updateStep(toolContext, 3); // 更新到第 3 步(STEP_EXPRESS_INFO_COLLECTOR)
...
//    }
    @Tool(description = "保存快递下单的寄送物品信息，如物品重量、体积、特殊要求、类型等，用于快递下单场景")
    public String expressInfo(
            @ToolParam(description = "寄送的快递信息，如 五本书、一件衣服、一部iphone15等") String expressInfo,
            ToolContext toolContext
    ) {
        log.info("【快递下单】快递物品信息：{}", expressInfo);
        saveInfoToContext(toolContext, KEY_EXPRESS_INFO, expressInfo);
        updateStep(toolContext, 4);
        return "✅ 快递物品信息已保存：" + expressInfo + "\n\n所有信息已收集完毕！可以调用 showExpressOrder 查看现在的订单信息";
    }
...
//    }
     ...
     * 保存数据到上下中
     *
     * _AGENT_STATE_FOR_UPDATE_ 的state信息在当前Node执行完毕之后，覆盖_AGENT_STATE_
     *
     * @param toolContext
     * @param key
     * @param value
     */
    private void saveInfoToContext(ToolContext toolContext, String key, String value) {
        ToolContextHelper.getStateForUpdate(toolContext).ifPresent(new Consumer<Map<String, Object>>() {
            @Override
            public void accept(Map<String, Object> objectMap) { // objectMap是toolContext中指定key（有几种，看源码）下的Object
        ...
        ToolContextHelper.getStateForUpdate(toolContext).ifPresent(context -> {
            ...
            switch (step) {
                case 0:// 重置进度
                    ...
                    break;
                case 1:
                    context.put(KEY_CURRENT_STEP, STEP_RECEIVE_INFO_COLLECTOR);
                    break;
                case 2:
                    context.put(KEY_CURRENT_STEP, STEP_SEND_INFO_COLLECTOR);
                    break;
                case 3:
                    context.put(KEY_CURRENT_STEP, STEP_EXPRESS_INFO_COLLECTOR);
                    break;
                case 4:
                    context.put(KEY_CURRENT_STEP, STEP_ORDER_CONFIRMATION);
            }
