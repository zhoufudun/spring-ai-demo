 ...
 */
@Component
public class SparkLiteModel_1 implements ChatModel {
    private final static Logger log = LoggerFactory.getLogger(SparkLiteModel_1.class);
    ...
    public ChatResponse call(Prompt prompt) {
        ...
        String body = restClient.post().body(POJOConvert.toReq(prompt, model)).retrieve().body(String.class);

        SparkPOJO.ChatCompletionChunk chatCompletionChunk = JsonUtil.fromStr(body, SparkPOJO.ChatCompletionChunk.class);
        List<Generation> generations = POJOConvert.generationList(chatCompletionChunk);

        return new ChatResponse(generations,POJOConvert.from(l,model,chatCompletionChunk));
    }
}
