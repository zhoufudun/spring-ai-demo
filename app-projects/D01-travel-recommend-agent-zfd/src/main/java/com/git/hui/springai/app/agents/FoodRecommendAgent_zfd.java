 ...
 */
@Service
public class FoodRecommendAgent_zfd {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(FoodRecommendAgent_zfd.class);
    public static final String NAME = "foodAgent";
    private final AgentService agentService;

    public FoodRecommendAgent_zfd(AgentService agentService) {
...
//        return output;
        String travel = state.getTravel();
        log.info("[callFoodAgent]: {}, travel: {}", state, travel);
//        log.info("FoodRecommendAgent_zfd: data=" + state.getAll());

        // 格式化输出
        BeanOutputConverter<TravelFoodRecommends> beanOutputConverter = new BeanOutputConverter<>(TravelFoodRecommends.class);
        // 提示词模板
        var prompt = """
                现在的天气是 {weather}
                请结合下面推荐的旅游游玩项目，帮我为每个项目推荐三种美食
        ...
        String render = promptTemplate.render(Map.of("weather", state.getWeather(), "travel", travel, "format", beanOutputConverter.getFormat()));

        String food = agentService.food(render);
        TravelFoodRecommends travelFoodRecommends = beanOutputConverter.convert(food);

        Map<String, Object> foodRecommendsMap = Map.of(TravelState.FOOD, travelFoodRecommends);
        log.info("Food Agent Output: {}", foodRecommendsMap);
        return foodRecommendsMap;
    }
