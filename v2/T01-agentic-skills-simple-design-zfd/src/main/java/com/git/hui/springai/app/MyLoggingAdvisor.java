    ...
    public ChatClientRequest before(ChatClientRequest chatClientRequest, AdvisorChain advisorChain) {
        ...
        if (this.showAvailableTools) {
            ...
            if (chatClientRequest.prompt().getOptions() instanceof ToolCallingChatOptions toolCallingChatOptions) {
                tools = toolCallingChatOptions.getToolCallbacks().stream().map(tc -> tc.getToolDefinition().name()).toList();
            }
        ...
        if (lastUserOrToolResponseMessage.getMessageType() == MessageType.TOOL) {
            ToolResponseMessage toolResponseMessage = (ToolResponseMessage) lastUserOrToolResponseMessage;
        ...
        } else if (lastUserOrToolResponseMessage.getMessageType() == MessageType.USER) {
            if (StringUtils.hasText(lastUserOrToolResponseMessage.getText())) {
                sb.append("\n - TEXT: ").append(first(lastUserOrToolResponseMessage.getText(), 1000));
            }
        }
